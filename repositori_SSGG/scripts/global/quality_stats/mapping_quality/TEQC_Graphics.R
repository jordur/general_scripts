...
t<-as.character(args[12]);#COVERAGE PER BASE
...
H<-hist(tt, breaks = 50, plot = FALSE);
t_limit<-max(H$intensities);
tab<-table(tt)
cs<-cumsum(rev(tab))
m<-max(H$counts)
cs.s<-cs*(m/sum(tab))
...
{
	sum<-sum+H$counts[i];
    if(sum >= percentile  && entry==0)
    {
        entry<-1;
        mt<-H$breaks[i];
    }
}
pdf("coverage_BASE_distribution.pdf");
plot(H, freq = TRUE, xlab = "Coverage", main = "Coverage Base Distribution",col="#336699",xlim=c(0,mt))
lines(x=names(cs.s), y=cs.s, col="red", lwd=2)
axis(side = 4, at = seq(0, m, length.out = 11), labels = seq(0,1, by = 0.1))
dev.off()
#######
...
plot(k$V3,cex=0.000000001,col=cols[2],ylim=c(0,1.5),ylab="% Bases",xlab="Chromosome",cex.axis=0.00000001,lwd=2,type="n")
...
axis(2)
legend(1.5,1.5,threshold,lty=c(1,1,1),lwd=c(2,2,2),col=cols_legend);
dev.off()
