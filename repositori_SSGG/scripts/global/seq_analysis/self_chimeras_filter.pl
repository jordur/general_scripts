...
my $others;
my $type;
warn ("# Contigs considered as putative chimera (seqID length chimType splitPos):\n");
	...
	if ($hit) {
		...
		if (@hsps && $hsps[0]->percent_identity >= $self_id_cutoff) {
			...
			if ($split < $length) {
				...
				warn join("\t",$seq_id, $length, $type, $split)."\n";
			}
				...
				printf STDERR ("%s\t%d\trepeated_blocks_(%.0f%%)\tdiscarded\n", $seq_id, $length, $tiling->length('query')/$length*100);
				next;
			}
			else {
				if ($hsps[0]->length('query')*2 >= $length/100*$self_cov_cutoff) {
