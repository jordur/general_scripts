...
sub usage {
	print "\nEste script crea un archivo tabulado con la relación de genes únicos y el número de posiciones encontradas en cada gen.\n";
        print "\nCOMO SE USA: sg_extraer_genes_unicos_y_numero_SNV_indels_por_gen.pl <fichero> \n";
	print "INPUT: fichero tabulado en el que la primera columna es el cromosoma, la segunda la coordenada de la variación y la columna 8 es en la que se encuentra el nombre del gen. Está pensado para realizar el análisis de herencia recesiva o dominante con los ficheros de entrega de resecuenciación dirigida.\n\n";
