...
#	$4 : output directory sufix (ending with /)
{
	...
	# Remove duplicates
	MarkDuplicates.jar I=$1$2/$3F3-F5-BC-Paired.bam O=$1$2/$3PE_nodup.bam M=$1$2/$3$4PE_nodup.log REMOVE_DUPLICATES=true TMP_DIR=$1$2/$3$4scratch

	# Filtering of the reads by quality values below 10
	samtools view -b -q 10 $1$2/$3PE_nodup.bam > $1$2/$3PE_Q10_nodup.bam
	
	# Sequences (with duplicates) get piled up against the reference
	#/share/apps/samtools-0.1.9/samtools pileup -cf /data/results/Solid0065/referencia_genoma_humano/GRch37.58/hg19.fa $1$2/$3F3-F5-BC-Paired.bam > $1$2/$3PE.pileup
	
	# Sequences (without duplicates) get piled up against the reference
	/share/apps/samtools-0.1.9/samtools pileup -cf /data/results/Solid0065/referencia_genoma_humano/GRch37.58/hg19.fa $1$2/$3PE_Q10_nodup.bam > $1$2/$3$4PE_Q10_nodup.pileup
	...
	for j in ${i[@]}; do
        	awk '{if ($1=="chr'${j}'") print $0}' $1$2/$3$4PE_Q10_nodup.pileup > $1$2/$3$4chr$j.pileup
	done
...
input_dir=("secondary/bioscope/pairing/" "secondary/bioscope/pairing_110225_O/" "secondary/bioscope/pairing_110301_O/" "secondary/bioscope/pairing_110302_O/" "secondary/bioscope/pairing_110301_O/" "secondary/bioscope110214/output/pairing_110302_O/" "secondary/bioscope110214/output/pairing_110303_O/" "secondary/bioscope110214/output/pairing_110202_O/" "secondary/bioscope20110214/output/pairing_110303_O/" "secondary/bioscope110214/output/pairing_110225_O/" "secondary/bioscope_110213/output/pairing_110302_O/" "secondary/bioscope_110207/output/pairing_110303_O/")
# the output_dir is based on the input_dir (and based dir), this means: root_dir/input_dir/samples_output_dir
output_dir="stats_PE_Q10_nodup/"
