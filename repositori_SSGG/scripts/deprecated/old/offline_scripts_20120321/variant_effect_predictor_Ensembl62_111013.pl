...
&main($config);
	...
	sub main {
		...
		#$config->{mlss_adaptor} = Bio::EnsEMBL::Registry ->get_adaptor('Multi', 'compara', 'MethodLinkSpeciesSet');
		#my $mlss_adaptor = $config->{reg}->get_adaptor("Multi", "compara", "MethodLinkSpeciesSet");
		#print $config->{mlss_adaptor},"\n";
		$config ->{mlss} = $config->{mlss_adaptor}->fetch_by_method_link_type_species_set_name("GERP_CONSERVATION_SCORE", "mammals");
		throw("Unable to find method_link_species_set") if (!defined($config ->{mlss}));
		...
		foreach my $tv(@{$new_vf->get_all_TranscriptVariations}) {
			...
			foreach my $tva(@{$tv->get_all_alternate_TranscriptVariationAlleles}) {
				...
				if(defined $config->{hgnc} && $gene) {
				  ...
				  if(scalar @entries) {
					 $extra = 'HGNC='.$entries[0]->display_id.';'; #	$extra .= 'HGNC='.$entries[0]->display_id.';';
				  }
				}
				...
				if(defined $config->{hgvs}) {
					$extra .= $tva->hgvs_coding if defined($tva->hgvs_coding);
					#$hgvs_cds = 'HGVSc='.$tva->hgvs_coding.';' if defined($tva->hgvs_coding); # 
					#$extra .= 'HGVSc='.$tva->hgvs_coding.';' if defined($tva->hgvs_coding);
					#$hgvs_proteina = 'HGVSp='.$tva->hgvs_protein.';' if defined($tva->hgvs_protein); #
				...
				my @split_new = split ("_",$new_vf->variation_name);
#################################

				my $slice_adaptor = $config->{reg}->get_adaptor(($config->{species}), 'core', 'Transcript');
				my $transcript_id=$tv->transcript->stable_id;

				my $tr_jc=$slice_adaptor->fetch_by_stable_id($transcript_id);
				my $translation = $tr_jc->translation;
				my $interpro_id;
				my $interpro_desc;

				if(!$translation)
				{
#					print "No Existe!!!\t$transcript_id\n";
				}
				else
				{
						...
						while ( my $pfeature = shift @{$pfeatures} )
						{
							...
							if(($aa_start >=$pfeature->start() && $aa_start<=$pfeature->end()) || ($aa_end >=$pfeature->start() && $aa_end<=$pfeature->end())){
								if ($pfeature->interpro_ac() && $pfeature->idesc()) {
									...
									if ($interpro_id =~ /$ipro_id/ || $interpro_desc =~ /$ipro_desc/) {
										next;
									}
									elsif (!$interpro_id && !$interpro_desc) {
										$interpro_id .= $pfeature->interpro_ac();
										$interpro_desc .= $pfeature->idesc();
									}
									else {
										$interpro_id .= ','.$pfeature->interpro_ac();
										$interpro_desc .= ','.$pfeature->idesc();
...
## Strand  :Strand information of a transcript
...
#throw("No Slice can be created with coordinates $seq_region:$seq_region_start-$seq_region_end") if (!$slice);
        ...
        my ($var1,$var2,$var3)=@_;
        #print $_[0],"---", $_[1],"--", $_[2],"\n";
        my $nslice = $config->{sa}->fetch_by_region('toplevel', $var1, $var2, ($var3 +1) );
