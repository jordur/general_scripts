...
for (my $a=0; $a<=$#mirbase2 ;$a++)
{
	...
	for (my $i=0;$i<=$#conteo;$i++)
	{
		...
		{
			print $mirbase[0]."_".$mirbase[1]."_".$mirbase[4]."_".$mirbase[2]."_".$mirbase[3],"\t",$conteo_split[1],"\t",$conteo_split[2],"\t",$conteo_split2[1],"\n";		
			$contador=$contador+$conteo_split2[1];
		}
	}
	print $mirbase[0]."_".$mirbase[1]."_".$mirbase[4]."_".$mirbase[2]."_".$mirbase[3],"=",$contador,"\n";
	$contador =0;
}
