...
sub usage {
	...
	print "INPUT: fichero tabulado en el que la primera coordenada es el cromosoma, la segunda la coordenada, la tercera, la base referencia, la cuarta, el genotipo encontrado.\n\n";
	print "OUTPUT: fichero con el formato 2,3687911,1,T/G y cuyo nombre es igual al del input con el prefijo 'SIFT_'. Se crean dos líneas por SNV, uno por cadena.\n\n";
        exit(1);
	}	
...
while (my $snv=<SNVS>)
{
	...
	my @ex = split(/\t/,$snv);
	$ex[0] =~ tr/chr//d;
	my $genotype = convertir_a_homocigoto($ex[2],$ex[3]);
	print OUTPUT "$ex[0],$ex[1],1,$ex[2]/$genotype\n$ex[0],$ex[1],-1,$ex[2]/$genotype\n";
}
