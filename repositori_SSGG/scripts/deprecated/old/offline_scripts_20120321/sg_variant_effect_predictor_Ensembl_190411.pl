...
print $out_file_handle
	"Uploaded_variation\tLocation\tGene\tTranscript\t",
	"Gene_name\tGene_description\tConsequence\tPosition_in_cDNA\tPosition_in_protein\t",
	"Amino_acid_change\tCorresponding_variation\tConservation_score\n";


# get adaptors
our $vfa = $reg->get_adaptor($species, 'variation', 'variationfeature');
our $tva = $reg->get_adaptor($species, 'variation', 'transcriptvariation');
#incluyo el adptor que se utilizara para los valores de conservacion GERP
our $mlss_adaptor = $reg->get_adaptor("Multi", "compara", "MethodLinkSpeciesSet");
our $mlss = $mlss_adaptor->fetch_by_method_link_type_species_set_name("GERP_CONSERVATION_SCORE", "mammals");
throw("Unable to find method_link_species_set") if (!defined($mlss));
our $cs_adaptor = $reg->get_adaptor("Multi", 'compara', 'ConservationScore');
...
while(<$in_file_handle>) {
  ...
  $line_number++;
  #print "leyendo el numero de linea $line_number \n";
  # header line?
  next if /^\#/;
  ...
  foreach my $sub_line(@{parse_line($_)}) {
	...
	if($whole_genome) {
		...
		push @new_vfs, $new_vf;
		#print "tengo todos los VariationFeatures\n";
	}
...
if(scalar @new_vfs) {
	#print "adquiriendo todas las variaciones en el slice\n";
	$tva->fetch_all_by_VariationFeatures(\@new_vfs);
#	print "tva --> ",$tva,"\n";
	&print_consequences(\@new_vfs,$out_file_handle);
}
	...
	foreach my $new_vf(@$vfs) {
		...
		foreach my $con(@{$new_vf->get_all_TranscriptVariations}) {
			foreach my $string(@{$con->consequence_type}) {
			  ...
			  #el numero de chr esta en el primer campo del variation_name asi que lo divido por "_"
			  my @num_chr=split("_",$new_vf->variation_name);
				print $out_file_handle
				#$con-> hgvs_coding, "\t",
				#$new_vf-> is_somatic,"\t",
				...
				($gene ? $gene->external_name.(defined $hgnc_name ? ";$hgnc_name" : "") : "-"), "\t",
				($gene ? $gene->description.(defined $hgnc_name ? ";$hgnc_name" : "") : "-"), "\t",
				$string, "\t",
				#($con->cdna_start ? $con->cdna_start.($con->cdna_end eq $con->cdna_start ? "" : "-".$con->cdna_end) : "-"), "\t",
				($con->cds_start ? $con->cds_start.($con->cds_end eq $con->cds_start ? "" : "-".$con->cds_end) : "-") ,"\t",
				...
				$existing_vf, "\t",
				&print_conservation_score ($num_chr[0], $new_vf->seq_region_start,$new_vf->seq_region_end),"\n";
			}
		}
	}
}
...
#sub anotacion_aminoacido {

#	my $string = shift;
#	my $sub_string = substr ($string,1,1);
#	print $sub_string," ---MISUB--- \n";
#	my @split_sub_string = split ($sub_string,	

#}


sub print_conservation_score {
...
#throw("No Slice can be created with coordinates $seq_region:$seq_region_start-$seq_region_end") if (!$slice);
	...
	my ($var1,$var2,$var3)=@_;
	#print $_[0],"---", $_[1],"--", $_[2],"\n";
	my $nslice = $sa->fetch_by_region('toplevel', $_[0], $_[1], $_[2]+1);
	my $display_size = ($nslice->end )- $nslice->start + 1;
	my $scores = $cs_adaptor->fetch_all_by_MethodLinkSpeciesSet_Slice($mlss, $nslice, $display_size);
	...
	foreach my $score (@$scores) {
			...
			return sprintf ("%.4f", $score->diff_score);
			}
    		}
		else
		{
			return "GERP score unavailable";
		}
	}
}
