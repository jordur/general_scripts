...
print "Chr\tStart\tEnd\tSize\tReference\tSample\tType\tGene_ID\tTranscript_ID\tEnsembl_prediction\tAmino acid change\tCorresponding Variation\tSIFT prediction\tSIFT score\tGene name\tDescription\tGenotypes\tRead distribution\tNon-redundant reads\tQ_SNV\tGERP conservation score\tOMIM_disease\tComments\n";
...
while($gen=<GENES>)
{
	...
	for($n = 0; $n<=$#array_anotacion;$n++)
	{
		...
		if($split_gen[0] eq $line_anotacion[7])
		{
