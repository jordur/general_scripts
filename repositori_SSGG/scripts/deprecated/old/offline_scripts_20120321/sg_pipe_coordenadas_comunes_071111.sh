...
#El input del script son ficheros pileup procedente de samtools mpileup. Es importante poner los ficheros pileup en un mismo directorio y dar la entrada como *pileup
echo $(date)
...
do
	...
	do
		...
		/share/apps/scripts/sg_extraer_SNPs_fichero_tabulado.pl /data/results/solid0065/info_resecuenciacion/intervalos_captura/BRCA/BRCA_Junior_v2.bed $sample\_bases_$i > $sample\_bases_N_rango_$i
		#-------------> Coge posiciones cubiertas y coverage correspondiente
		awk '{print $2"\t"$4}'  $sample\_bases_N_rango_$i > $sample\_$i
		...
		#------------->  Cambiar intervalos de captura
		/share/apps/scripts/resecuenciacion_dirigida/deleciones-duplicaciones/sg_convertir_intervalo_N_columna_posiciones.pl /data/results/solid0065/info_resecuenciacion/intervalos_captura/BRCA/BRCA_Junior_v2.bed | sort -u -n >  posiciones_$i
