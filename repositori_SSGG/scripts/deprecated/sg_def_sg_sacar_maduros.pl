...
sub usage
{
    ...
    print " Input: 1) fichero tabulado del conteo de miRNAs 2) fichero con miRNAs maduros ej: mir2<TAB>chr<TAB>inicio<TAB>final<TAB>orientacion  3) nombre del cromosoma\n";
    exit(1);
}
...
for (my $a=0; $a<=$#mirbase2 ;$a++)
{
	...
	for (my $i=0;$i<=$#conteo;$i++)
	{
		...
		#print $mirbase[4],"\n";#" igual ",$mirbase[4],"\n";
		if ((((abs($conteo_split[1]-$mirbase[2])) <= 10) && ($conteo_split[3] eq $mirbase[4])) || (((abs($conteo_split[2]-$mirbase[3])) <= 10) && ($conteo_split[3] eq $mirbase[4])) || (($conteo_split[1]>=$mirbase[2]) && ($conteo_split[2]<=$mirbase[3]) && ($conteo_split[3] eq $mirbase[4])))
		{
		...
		elsif ((((abs($conteo_split[1]-$mirbase[2])) <= 10) && ($conteo_split[3] ne $mirbase[4])) || (((abs($conteo_split[2]-$mirbase[3])) <= 10) && ($conteo_split[3] ne $mirbase[4])) || (($conteo_split[1]>=$mirbase[2]) && ($conteo_split[2]<=$mirbase[3]) && ($conteo_split[3] ne $mirbase[4])))
                {
