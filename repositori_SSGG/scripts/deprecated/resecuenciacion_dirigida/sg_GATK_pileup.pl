...
open (PROBE, "< $probes_id") or die "Can't open $probes_id\n";

while (my $line = <PROBE>) {
	chomp $line;
	my ($chr, $start, $end) = split (/\s+/, $line);
	$chr =~ s/chr//;
	$probes{$chr}{$start}{end} = $end;
}
...
print IGV "chr\tstart\tend\tprobe\tdepth\n";
...
PROBE:	foreach my $start (keys %{$probes{$chr}}) {
		if ($position >= $start && $position <= $probes{$chr}{$start}{end}) {
			if ($data[9] eq "./.") {
				$depth = 0;
			}
			else {
			...
			print IGV "$chr\t$position\t",$position + 1,"\t",$chr."_".$position,"\t$depth\n";
			last PROBE;
		}
	}
}
