...
#       java -jar /share/apps/GenomeAnalysisTK-1.0.5777/GenomeAnalysisTK.jar -T TableRecalibration  -R /data/results/Solid0065/BF42_BRCA_muestrasJB/datos_partida/referencia/refOK.fasta  -I stepII_realigner.bam -o stepII_realigner_sort_Recalibrate.bam -recalFile recal.csv
        ...
        java -jar /share/apps/GenomeAnalysisTK-1.0.5777/GenomeAnalysisTK.jar -T UnifiedGenotyper -R /data/results/Solid0065/referencia_genoma_humano/GRch37.58/hg19.fa  -glm SNP -nt 8  -stand_call_conf 40.0 -stand_emit_conf 20.0 -dcov 200 -I input_PicardSort_nodup_Q1.bam  -o snps_GATK.vcf

	echo "------------Variant calling Fragment--------"
	java -jar /share/apps/GenomeAnalysisTK-1.0.5777/GenomeAnalysisTK.jar -T UnifiedGenotyper -R /data/results/Solid0065/referencia_genoma_humano/GRch37.58/hg19.fa  -glm SNP -nt 8  -stand_call_conf 40.0 -stand_emit_conf 20.0 -dcov 200 -I F3_input_PicardSort_nodup_Q1.bam  -o F3_snps_GATK.vcf
	...
	/share/apps/GenomeAnalysisTK-1.0.5777/GenomeAnalysisTK.jar -T VariantFiltration -R /data/results/Solid0065/referencia_genoma_humano/GRch37.58/hg19.fa -filter "MQ0 >= 4 && ((MQ0 /(1.0*DP))> 0.1)" -filter "QUAL < 30.0" -filter "SB > 0.1" -filter "QD < 5.0" -filter "HRun >= 4"  -filterName HARD -filterName QUAL_FILTER -filterName SB_FILTER -filterName QD_FILTER -filterName HRUN_FILTER  -cluster 3 -window 10 -B:variant,VCF F3_snps_GATK.vcf -o F3_snps_GATK_filtrados.vcf
	...
	/share/apps/GenomeAnalysisTK-1.0.5083/GenomeAnalysisTK.jar -T VariantFiltration -R /data/results/Solid0065/referencia_genoma_humano/GRch37.58/hg19.fa  -filter "MQ0 >= 4 && ((MQ0 /(1.0*DP))> 0.1)" -filter "QUAL < 30.0" -filter "SB > -1.0" -filter "QD < 2.0"  -filterName HARD -filterName QUAL_FILTER -filterName SB_FILTER -filterName QD_FILTER  -B:variant,VCF F3_indels_GATK.vcf -o F3_indels_GATK_filtrados.vcf
	...
	#/share/apps/scripts/sg_integration_bioscopeVCF_format.pl $3 > bioscope_INDELS.vcf
	#/share/apps/scripts/sg_integration_bioscopeVCF_format.pl $4 > F3_bioscope_INDELS.vcf

	/share/apps/scripts/resecuenciacion_dirigida/gff_Bioscope_VCF/sg_convertir_GFF_en_VCF.sh $3 > bioscope_INDELS.vcf
	/share/apps/scripts/resecuenciacion_dirigida/gff_Bioscope_VCF/sg_convertir_GFF_en_VCF.sh $4 > F3_bioscope_INDELS.vcf
...
anotacion ()
{
        ...
        /share/apps/scripts/sg_variant_effect_predictor_Ensembl_190411.pl -i cambios_SNPs_integrados.vcf  -o variant_effect_output.txt_SNPs_tmp1 -format vcf -b 1000
	/share/apps/scripts/sg_variant_effect_predictor_Ensembl_190411.pl -i cambios_INDELS_integrados.vcf -o variant_effect_output.txt_INDELS_tmp1 -format vcf -b 1000


        /share/apps/scripts/sg_sustituir_porespacio_annotacion.pl variant_effect_output.txt_SNPs_tmp1 > variant_effect_output.txt_SNPs_tmp2
	/share/apps/scripts/sg_sustituir_porespacio_annotacion.pl variant_effect_output.txt_INDELS_tmp1 > variant_effect_output.txt_INDELS_tmp2
	...
	for c in ${i[@]}; do
		...
		awk '{if ($1=="chr'$c'") print $0}' cambios_INDELS_integrados.vcf  | sort -k2n > $c\_INDELS.vcf
                ...
                sed 's/_/\t/gi' variant_effect_output.txt_SNPs_tmp2 > variant_effect_output.txt_SNPs_tmp
		sed 's/_/\t/gi' variant_effect_output.txt_INDELS_tmp1 > variant_effect_output.txt_INDELS_tmp
	...
	if [ $5 == "Exoma38_hg18" ]; then
                cp  cambios_anotados_tmp2_SNPs.xls > Exoma38_hg18_SNPs.xls
		cp  cambios_anotados_tmp2_INDELS.xls > Exoma38_hg18_INDELS.xls
        fi

        if [ $5 == "Exoma38_hg19" ]; then
                cp cambios_anotados_tmp2_SNPs.xls > Exoma38_hg19_SNPs.xls
		cp cambios_anotados_tmp2_INDELS.xls > Exoma38_hg19_INDELS.xls
        fi
	if [ $5 == "Exoma50" ]; then
                cp cambios_anotados_tmp2_SNPs.xls > Exoma50_SNPs.xls
		cp cambios_anotados_tmp2_INDELS.xls > Exoma50_INDELS.xls
        fi
