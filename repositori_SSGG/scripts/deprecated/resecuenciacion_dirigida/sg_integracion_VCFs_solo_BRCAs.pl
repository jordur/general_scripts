...
while($line=<FILE>)
{
	...
	if ($explode[0] eq "chr17")
	{
		$rev3= reverse $explode[3];
		$rev4= reverse $explode[4];
		$rev3=~tr/ACGTacgt/TGCAtgca/;
		$rev4=~tr/ACGTacgt/TGCAtgca/;
		$explode[3]=$rev3;
		$explode[4]=$rev4;
		$explode[1]=$explode[1]-length($explode[3])+1;
