...
for ((k=0;k<${number_commands};k+=1)); do
	...
	if [ ! -e bash_job ]; then
		...
		echo "Problem with existent file bash_job.sh!! Please rename the existent file"
		exit
	fi
	...
	if [ "$interpreter" == "bash" ]; then
		...
		job_info=`submit_bash_job -N command$k bash_job | awk '{print $3}'`
	else 
		if [ "$interpreter" == "perl" ]; then
			echo "Launching command:      submit_perl_job ${proc_commands[ ( $k ) ]}"
			job_info=`submit_perl_job -N command$k bash_job | awk '{print $3}'`
		else
			echo "Launching command:      submit_python_job ${proc_commands[ ( $k ) ]}"
			job_info=`submit_python_job -N command$k bash_job | awk '{print $3}'`
		fi
	fi
	echo "on $exec_time"
	...
	if [ $job_info != "" ]; then
		...
		echo "Problem with command ${proc_commands[ ( $k ) ]}. The rest of commands will continue executing."
	fi
done
...
job_id=`submit_bash_job $options -N watcher-job watcher-job | awk '{print $3}'`
