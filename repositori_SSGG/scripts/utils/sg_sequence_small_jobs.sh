...
# Displays the script usage
{
        ...
        echo
        echo "* Options: *"
	echo " -N job_name		name of the sequence jobs that will be launched"
...
for ((k=0;k<${number_commands};k+=1)); do
	...
	if [ "$interpreter" == "bash" ]; then
		job_info=`submit_small_bash_job $options -N $jobname$k bash_job | awk '{print $3}'`
		echo "Launching job with ID $job_info:      submit_small_bash_job $options ${proc_commands[ ( $k ) ]}"
	else 
		if [ "$interpreter" == "perl" ]; then
			job_info=`submit_small_perl_job $options -N $jobname$k bash_job | awk '{print $3}'`
			echo "Launching job with ID $job_info:      submit_small_perl_job $options ${proc_commands[ ( $k ) ]}"
		else
			job_info=`submit_small_python_job $options -N $jobname$k bash_job | awk '{print $3}'`
			echo "Launching job with ID $job_info:      submit_small_python_job $options ${proc_commands[ ( $k ) ]}"
		fi
	fi
	echo "on $exec_time"
...
echo "Last_job_ID: $job_info"
export Last_job_ID=$job_info
