...
for ((k=0;k<${number_commands};k+=1)); do
	...
	if [ "$interpreter" == "bash" ]; then
		...
		job_info=`submit_small_bash_job -N command$k bash_job | awk '{print $3}'`
	else 
		if [ "$interpreter" == "perl" ]; then
			echo "Launching command:      submit_perl_job ${proc_commands[ ( $k ) ]}"
			job_info=`submit_small_perl_job -N command$k bash_job | awk '{print $3}'`
		else
			echo "Launching command:      submit_python_job ${proc_commands[ ( $k ) ]}"
			job_info=`submit_small_python_job -N command$k bash_job | awk '{print $3}'`
		fi
	fi
	echo "on $exec_time"
