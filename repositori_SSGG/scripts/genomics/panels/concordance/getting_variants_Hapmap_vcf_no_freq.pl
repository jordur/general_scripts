...
sub process ($) {
  	my ($chr_id) = @_;
	open (OUT, "> $output_id.chr$chr_id.snv") or die "Can't create $output_id.chr$chr_id.snv\n";
	print "Writing output file for chr$chr_id... ".localtime()."\n";
	print OUT "#CHR\tPOSITION\tREF_AL\tVAL_AL\tVAL_AL_FREQ\tSNP_ID\t";
	my $first_line = `grep -m 1 \"^#CHROM\" $input_id.chr$chr_id.vcf`;
	chomp $first_line;
	my @samples = split (/,/, $samples_id);
	my @pos;
	foreach my $sample (@samples) {
		print OUT "$sample\_genotype\t$sample\_depth";
	...
	while (my $line = <TMP>) {
		...
		my $allele_freq = (&getfreq($data[7]) ||'0');
		print OUT "$data[0]\t$data[1]\t$data[3]\t$data[4]\t$allele_freq\t$data[2]\t";
		foreach my $col (@pos) {
			my $genotype = &getgenotype($data[3], $data[4], $data[$col]);
			my $depth = &getdepth($data[3], $data[4], $data[$col]);
			print OUT "$genotype\t$depth\t";
		}
		print OUT "\n";
	}
	close OUT;
...
sub getsnps ($$$$) {
	...
	open (VCF, "gunzip -c $input_id.chr$chr_id.vcf.gz|") or die "Can't open $input_id.chr$chr_id.vcf\n";
