	...
	if ($line =~ m/^#/ || length($data[3]) == length($data[4])) {
		...
		if ($print_line) {
			if ($chr eq $last_chr && ($pos - $last_pos) <= abs(length($data[3]) - length($data[4])) && $type eq $last_type){
		        ...
		        if ($freq == 1) { $var = sprintf("%.0f",$depth);}
        		else {$var = sprintf("%.0f",($freq * $depth)/ (1 - $freq));}
				...
				if ($last_freq == 1) {$last_var = sprintf("%.0f", $last_depth);}
		        else {$last_var = sprintf("%.0f",($last_freq * $last_depth)/ (1 - $last_freq));}
