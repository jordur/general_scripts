...
#open (LOG, "> parsing_indels.log") or die "Can't create parsing_indels.log\n";
...
while (my $line = <VCF>) {
	...
	if ($line =~ m/^#/) {
		...
		if ($data[7] =~ m/indel/i) {
				...
				my $string;
				my $ref = (split (//, $data[3]))[0];
				#print LOG "$ref\t";
#				if (length($data[3]) > length($data[4])) { #deletion
					#print LOG "deletion $data[3] $data[4]\n";
#					$string = $data[3];
					#$string =~ s/$data[4]//;
					#$data[3] = $ref.$string;
#					$data[3] = $string;
#					$data[4] = $ref;
					#print LOG "deletion $string $data[3] $data[4]\n";
#				}
#				elsif (length($data[4]) > length($data[3])) { #insertion
					#print LOG "insertion $data[3] $data[4]\n";
#					$string = $data[4];
					#$string =~ s/$data[3]//;
#					$data[4] = $ref.$string;
#					$data[4] = $string;
#					$data[3] = $ref;
					#print LOG "insertion $string $data[3] $data[4]\n";
#				}
