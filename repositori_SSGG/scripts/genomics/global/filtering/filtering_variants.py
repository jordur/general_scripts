...
- to increase legibility when using more filters, arguments should follow the filter name
- it is good to specify the filters explicitly by name, because the order of filtering can matter # solution
- change the command syntax to vcf_filter.py --core-options input filter1 --filter1-args filter2 filter3
- parse the core program options with parse_known_args
- use add_argument_group for filters (subparsers won't work, they require the second command in argv[1])
...
def create_filt_parser(name):
    parser = argparse.ArgumentParser(description='Parser for %s' % name, add_help=False)
...
def create_core_parser():
    ...
    parser = argparse.ArgumentParser(description='Filter a VCF file', add_help=False,
...
""")
    ...
    # parser.add_argument('filters', metavar='filter', type=str, nargs='*', default=None, help='Filters to use')
    ...
    parser.add_argument('--output', action='store', default=sys.stdout, help='Filename to output [STDOUT]')
    parser.add_argument('--no-filtered', action='store_true', help='Output only sites passing the filters')
    ...
    parser.add_argument('rest', nargs=argparse.REMAINDER, help=argparse.SUPPRESS)
    parser.add_argument('-d', '--debug', action='store_true', default=False, help='allows remote debugging')
    return parser
