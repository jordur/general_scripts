...
# @Desc: Variant Effect Predictor launcher for VEP 71,
# @Version: Ensembl71 launcher v1.0
# @Status: Develop environment
# @Author: Guillermo Marco Puche
# @Last modification: 10/06/13
# @Contributors: Arbol 
########################################################
...
sub main {
	...
	print "\n=================================================================
	Variant Effect Predictor Launcher 71\n";
	print "\t",`date`;
	if (not defined $options{i} or not defined $options{o} or not defined $options{t}){
		print "\t!!! YOU MUST SPECIFY ALL FLAGS !!!\n\n";
		print "-i *.vcf file USING ABSOLUTE PATH !!! ie: /home/data/file.vcf\n";
		print "-o output_file USING ABSOLUTE PATH !!! ie: /home/results/output.vep\n";
		print "-t number_of_threads ie: -t 4\n";
		print "-b buffer size ie: -b 15000. Default value is 5000 variants loaded on memory.\n";
		print "=================================================================\n\n";
		exit;
	}
...
my ($gmaf, $gmaf_index, @cols, $cols);
	...
	while (<VEP_OUTPUT>){
		...
		if ($_ =~ /^#/){
			...
			s/HGNC/HGNC_symbol/, s/HGVSc/HGVSc_name/, s/HGVSp/HGVSp_name/, s/INTRON/Intron/, 
			...
			s/MOTIF_NAME/Regulatory_Motif_name/, s/MOTIF_POS/Regulatory_Motif_position/,s/HIGH_INF_POS/Regulatory_High_Inf_Pos/,
...
sub launch_vep {
	...
	#We launch VEP
	system("$vep_path -i $vcf_input -o $output -config $vep_template -plugin vcf_input,$vcf_gz -plugin biobase,$vcf_gz -fields $fields -fork $threads -buffer_size $buffer_size");
	#system("$vep_path -i $vcf_input -o $output -config $vep_template -plugin vcf_input,$vcf_gz -plugin biobase,$vcf_gz -fork $threads -buffer_size $buffer_size");
	...
	print "Parsed VEP output file: $output\n";
	print "=================================================================\n"; 
}
