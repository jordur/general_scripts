...
sub get_header_info {
    ...
    return {
        Conservation_score => "The conservation score for this site (method_link_type=\"".
...
sub run {
    ...
    unless (exists $bvf->{_conservation_score}) {
        ...
        if (@$scores > 0) {
            ...
            if (@$scores > 0) {
                my @tot_score;
                my ($conservation, $element);
                                	
                foreach my $i (@$scores){
                	$element = sprintf "%.3f", $i->diff_score;
                	push(@tot_score, $element);
                }

				#@tot_score = $_->diff_score for $scores;
                
                foreach my $individual (@tot_score){
                ...
                $bvf->{_conservation_score} = $conservation;
            }
            else {
    ...
    if (defined $bvf->{_conservation_score}) {
        return {
            Conservation_score => $bvf->{_conservation_score}
        };
    }
    else {
        return {};
