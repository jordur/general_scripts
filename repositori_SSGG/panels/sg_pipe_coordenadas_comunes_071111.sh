...
do
	...
	do
		...
		awk '{if ($1=="'$i'") print $0}' ../$sample  | sort -u -k2n > $sample\_bases_$i
		
		# Diseño con zonas splicing
		awk '{if ($1=="'$i'") print $0}' /home/szuniga/macropanel/convertir_pileup/macropanel-mas30/macropanel | sort -u > xxx_$i
        ../sg_convertir_intervalo_N_columna_posiciones.pl xxx_$i | sort -u -k2n | awk '{print $1"\t"$2"\t"$8}' > xxxX_$i
        ../sg_intervalos_comunes_071111.pl xxxX_$i covered > chr$i.bed
	
		#Saca posiciones en rango 
		#-------------> Cambiar intervalos de captura
		/share/apps/scripts/sg_extraer_SNPs_fichero_tabulado.pl chr$i.bed $sample\_bases_$i > $sample\_bases_N_rango_$i
		...
		#------------->  Cambiar intervalos de captura
		../sg_convertir_intervalo_N_posicion.pl chr$i.bed | sort -u -n >  posiciones_$i
...
do
	...
	if [[ -s posiciones_$x ]] ; then
		...
		../sg_coordenadas_comunes_011111.pl ${arr[@]} posiciones_$x
...
do
	...
	if [[ -s cromosoma_$fin ]] ; then
		...
		../sg_intervalos_comunes_071111.pl matriz_comparacion_entre_muestras_chr$fin.tab not_covered > intervalos_$fin
	fi
done
