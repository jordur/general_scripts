...
my $out_format = 'fastq-illumina';

## process input options
GetOptions('f|format=s' => \$out_format);
die "pass a fasta and a fasta-quality file\n"
  unless (@ARGV);

## exit script if input format not defined below
die "format should be one of fastq-sanger, fastq-illumina or fastq-solexa\n"
  unless ($out_format eq 'fastq-sanger' ||
	  $out_format eq 'fastq-illumina' ||
	  $out_format eq 'fastq-solexa');

## process input file names for use later
...
my $in_qual_obj =
  Bio::SeqIO->new( -file   => $qual_infile,
		   -format => 'qual');
my $out_fastq_obj =
  Bio::SeqIO->new( -format => $out_format);
